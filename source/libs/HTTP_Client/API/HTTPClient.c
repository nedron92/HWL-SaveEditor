...
// Returns      : long unsigned int - status
// Last updated : 01/09/2005
//
///////////////////////////////////////////////////////////////////////////////

#ifdef _WIN32
long unsigned int HTTPOSInit(long unsigned int nState // Requested operation        
                  // nState could be:
                  // 0 = Shutdown sockets       
                  // 1 = Turn on sockets        
                  )
...
HTTP_SESSION_HANDLE  HTTPClientOpenRequest (HTTP_CLIENT_SESSION_FLAGS Flags)
{
    ...
    long unsigned int         nAllocationSize;             // Size of the dynamically allocated buffer
    ...
    HTTPClientSetVerb((long unsigned int)pHTTPSession,(HTTP_VERB)HTTP_CLIENT_DEFAULT_VERB);    // Default HTTP verb
    ...
    // Return an allocated session pointer (cast to long unsigned int first)
...
long unsigned int  HTTPClientReset (HTTP_SESSION_HANDLE pSession)
{
...
long unsigned int  HTTPClientSetDebugHook (HTTP_SESSION_HANDLE pSession,
...
long unsigned int  HTTPClientSetVerb (HTTP_SESSION_HANDLE pSession,
...
long unsigned int  HTTPClientSetAuth (HTTP_SESSION_HANDLE pSession,
...
long unsigned int HTTPClientSetProxy (HTTP_SESSION_HANDLE pSession,
...
long unsigned int HTTPClientSetCredentials (HTTP_SESSION_HANDLE pSession, 
...
long unsigned int HTTPClientAddRequestHeaders (HTTP_SESSION_HANDLE pSession,
                                    CHAR *pHeaderName,      // [IN] The Headers name
                                    CHAR *pHeaderData,      // [IN] The headers data
                                    BOOL nInsert)           // [IN] Reserved could be any
{
    
    long unsigned int  nRetCode;
    long unsigned int  nHeaderLength,nDataLength;
    P_HTTP_SESSION pHTTPSession = NULL;
...
long unsigned int HTTPClientSendRequest (HTTP_SESSION_HANDLE pSession,
                              CHAR *pUrl,           //  [IN] The requested URL
                              VOID *pData,          //  [IN] Data to post to the server
                              long unsigned int nDataLength,   //  [IN] Length of posted data
                              BOOL TotalLength,     //  [IN] 
                              long unsigned int nTimeout,      //  [IN] Operation timeout
                              long unsigned int nClientPort)   //  [IN] Client side port 0 for none
{
    
    long unsigned int          nRetCode;               // Function call return code
    long unsigned int          nBytes;                 // Bytes counter (socket operations)
    long unsigned int          nUrlLength;             // Length of the given Url
...
long unsigned int HTTPClientRecvResponse (HTTP_SESSION_HANDLE pSession,
                               long unsigned int nTimeout) // [IN] Timeout for the operation
{
    
    long unsigned int          nRetCode;               // Function return code
    P_HTTP_SESSION  pHTTPSession = NULL;    // Session pointer
...
long unsigned int HTTPClientWriteData (HTTP_SESSION_HANDLE pSession, 
                            VOID *pBuffer, 
                            long unsigned int nBufferLength,
                            long unsigned int nTimeout)
{
    
    long unsigned int          nRetCode     = HTTP_CLIENT_SUCCESS;
...
///////////////////////////////////////////////////////////////////////////////
                            ...
                            long unsigned int HTTPClientReadData (HTTP_SESSION_HANDLE pSession,
                                VOID *pBuffer,           // [IN OUT] a pointer to a buffer that will be filled with the servers response
								long unsigned int nBytesToRead,     // [IN]     The size of the buffer (numbers of bytes to read)
								long unsigned int nTimeout,         // [IN]     operation timeout in seconds
								long unsigned int *nBytesRecived)   // [OUT]    Count of the bytes that ware received in this operation
                            {
                                
								long unsigned int          nBytes = 0;
								long unsigned int          nRetCode = 0;
...
long unsigned int HTTPIntrnSetURL (P_HTTP_SESSION pHTTPSession,
                        CHAR *pUrl,         // [IN] a null terminated string containing the Url we should retrieve
                        long unsigned int nUrlLength)  // [IN] The length the Url string
{
    
    long unsigned int         nUrlOffset;      // Offset in bytes within the Url string
...
long unsigned int HTTPIntrnResizeBuffer (P_HTTP_SESSION pHTTPSession,
                              long unsigned int nNewBufferSize)    // [IN] The new (and larger) buffer size
{
    
    CHAR                *pPtr           = NULL;
    long unsigned int              nCurrentBufferSize;
...
long unsigned int HTTPIntrnHeadersRemove (P_HTTP_SESSION pHTTPSession,
...
long unsigned int HTTPIntrnHeadersAdd (P_HTTP_SESSION pHTTPSession,
                            CHAR *pHeaderName,   // [IN] The header's name
                            long unsigned int nNameLength,  // [IN] Name length
                            CHAR *pHeaderData,   // [IN] The Header's data
                            long unsigned int nDataLength)  // [IN] Data length
{
    CHAR            *pPtr;
    long unsigned int          nProjectedHeaderLength;
    long unsigned int          nProjectedBufferLength;
...
long unsigned int HTTPIntrnConnectionOpen (P_HTTP_SESSION pHTTPSession)
{
    ...
    long unsigned int           nNullOffset;                        // a helper value to null terminate a given string
...
long unsigned int HTTPIntrnSend (P_HTTP_SESSION pHTTPSession,
                      CHAR *pData,            // [IN] a pointer to the data to be sent
                      long unsigned int *nLength)        // [IN OUT] Length of data to send and the transmitted bytes count
{
    ...
    do
    {
        ...
        while(1)
        {
            ...
            {
                FD_CLR((long unsigned int)pConnection->HttpSocket,&pConnection->FDError);
                *(nLength) = 0;
...
long unsigned int HTTPIntrnRecv (P_HTTP_SESSION pHTTPSession,
                      CHAR *pData,        // [IN] a pointer for a buffer that receives the data
                      long unsigned int *nLength,    // [IN OUT] Length of the buffer and the count of the received bytes
                      BOOL PeekOnly)      // [IN] State if we should only peek the socket (default is no)
{
    ...
    do
    {
        ...
        while(1)
        {
            ...
            {
                // Clear the event 
                FD_CLR((long unsigned int)pConnection->HttpSocket,&pConnection->FDRead);
...
long unsigned int HTTPIntrnGetRemoteChunkLength (P_HTTP_SESSION pHTTPSession)
{
    ...
    long unsigned int          nBytesRead = 1;
    long unsigned int          nRetCode = HTTP_CLIENT_SUCCESS;
    long unsigned int          nBytesCount = 0;
...
long unsigned int HTTPIntrnHeadersFind (P_HTTP_SESSION pHTTPSession,CHAR *pHeaderName,
                             ...
                             long unsigned int nOffset)         //  [IN]  Optionaly privide an offset to start looking from
{
    CHAR           *pHeaderEnd;
    CHAR           Header[HTTP_CLIENT_MAX_HEADER_SEARCH_CLUE]; // To-Do: Use pointers insted of fixed length
    long unsigned int         nLength;
    long unsigned int         nRetCode = HTTP_CLIENT_ERROR_HEADER_NOT_FOUND;
...
{
    long unsigned int      nRetCode = HTTP_CLIENT_SUCCESS;   // Function call return code
    long unsigned int      nBytes = 32;
    long unsigned int      nTotalBytes = 0;
    CHAR        ErrorPage[32];
...
long unsigned int HTTPIntrnHeadersParse (P_HTTP_SESSION pHTTPSession)
{
    ...
    long unsigned int          nTokenLength = 0;                           // Length of the parsed token
    long unsigned int          nRetCode = HTTP_CLIENT_SUCCESS;             // a function return code value
    long unsigned int          nOffset = 0;                                // Bytes offset (strings comperision)
...
long unsigned int HTTPIntrnParseAuthHeader(P_HTTP_SESSION pHTTPSession)
{
    ...
    if(pHTTPSession->HttpAuthHeader.HTTP_AUTH_SCHEMA != (long unsigned int)pHTTPSession->HttpCredentials.CredAuthSchema)
    {
...
long unsigned int HTTPIntrnHeadersSend(P_HTTP_SESSION pHTTPSession,
...
long unsigned int HTTPIntrnAuthSendBasic (P_HTTP_SESSION pHTTPSession)
{
    ...
    long unsigned int      nSegmentLength;
    long unsigned int      nRetCode;
    ...
    long unsigned int      nSrcLength, nDestLength;
    CHAR*       pPtr;
...
long unsigned int HTTPIntrnAuthSendDigest (P_HTTP_SESSION pHTTPSession)
{
    ...
    long unsigned int      nSegmentLength;
    long unsigned int      nRetCode;
    long unsigned int      nAlgType = 0; // a flag for the algorithem type (default to MD5)
...
{
    long unsigned int nActionTimeout; // For restoring a parameter after this reset
    long unsigned int nAllocationSize;
...
long unsigned int HTTPIntrnHeadersReceive (P_HTTP_SESSION pHTTPSession,
                                long unsigned int nTimeout)        // [IN] Timeout for the operation
...
BOOL HTTPIntrnSessionEvalTimeout(P_HTTP_SESSION pHTTPSession)
{
	...
	return FALSE;
#else
    long unsigned int nElapsedTime;    // Integer for calculating the elapsed time
