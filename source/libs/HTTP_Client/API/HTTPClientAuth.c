...
void HTTPMD5ByteReverse(unsigned char *buf, unsigned longs)
{
    long unsigned int t;
    do {
        t = (long unsigned int) ((unsigned) buf[3] << 8 | buf[2]) << 16 |
            ((unsigned) buf[1] << 8 | buf[0]);
        *(long unsigned int *) buf = t;
        buf += 4;
    } while (--longs);
}
#endif
...
void HTTPMD5Update(struct MD5Context *ctx, unsigned char const *buf, unsigned len)
{
    ...
    t = ctx->bits[0];
    if ((ctx->bits[0] = t + ((long unsigned int) len << 3)) < t)
    ...
    if (t) {
        ...
        HTTPMD5Transform(ctx->buf, (long unsigned int *) ctx->in);
        buf += t;
        len -= t;
    }
...
void HTTPMD5Final(unsigned char digest[16], struct MD5Context *ctx)
{
    ...
    *p++ = 0x80;
	...
	((long unsigned int *) ctx->in)[14] = ctx->bits[0];
	((long unsigned int *) ctx->in)[15] = ctx->bits[1];
...
void HTTPMD5Transform(long unsigned int buf[4], long unsigned int const in[16])
{
    register long unsigned int a, b, c, d;
