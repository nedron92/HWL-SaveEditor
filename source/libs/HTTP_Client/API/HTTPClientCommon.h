...
#define HTTP_CLIENT_EOS                     1000        // HTTP end of stream message
    ...
    {
        long unsigned int        HTTPStatusCode;                 // HTTP Status code (200 OK)
        long unsigned int        RequestBodyLengthSent;          // Total bytes sent (body only)
        long unsigned int        ResponseBodyLengthReceived;     // Total bytes received (body only)
        long unsigned int        TotalResponseBodyLength;        // as extracted from the �content-length" header
